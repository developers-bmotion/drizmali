...
( function( $ ) {
                ...
                if ($scope.find('.ae-acf-repeater-item').length) {
                    ...
                    adata = {
                        ...
                        on: {
                            ...
                            click : function () {
                                var clickedSlide = mswiper.clickedSlide;
                                var wrapper = $(clickedSlide).find('.ae-link-yes');
                                if(wrapper.length == 0){
                                    return;
                                }else{
                                    var url  = jQuery(wrapper).data( 'ae-url' );
                                    if(url !== undefined){
    ...
    var _ias = function ( $scope , $, grid ) {
        ...
        var $ias_obj = ias.infiniteScroll({
            path: '.next',
            append: wclass + ' .ae-post-list-item',
    ...
    var ACFRepeaterTabsHandler = function( $scope, $ ) {

        var defaultActiveTab = $scope.find( '.ae-acf-repeater-tabs' ).data( 'active-tab' ),
            $tabsTitles = $scope.find( '.ae-acf-repeater-tab-title' ),
            $tabs = $scope.find( '.ae-acf-repeater-tab-content' ),
            $active,
            $content;
        ...
        $tabsTitles.on( 'click', function() {
            activateTab( this.dataset.tab );
            jQuery(".elementor-widget-ae-acf-repeater .swiper-container").each(function(elem, target) {
    ...
    var ACFRepeaterAccordionHandler = function ( $scope , $ ) {
        $title = $scope.find('.elementor-tab-title.ae-tab-title');
        $speed = $scope.find('ae-accordion').data('transition-speed');
