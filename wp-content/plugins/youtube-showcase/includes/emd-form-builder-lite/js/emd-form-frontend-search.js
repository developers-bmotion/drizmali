	...
	$.fn.Paging = function (form_name,beg){
		...
		var load_posts = function(div_id,entity,view,form,app){
			$.ajax({
				...
				cache: false,
				async: false,
				data: {action:'emd_form_builder_pagenum',pageno: page,entity:entity,view:view,form:form,app:app,nonce:emd_form_vars.nonce},
	...
	if(emd_form_vars.enable_ajax){
		$(document).on('click','.emd-form-submit',function(event){
			...
			$.ajax({
				type: 'POST',
				url:emd_form_vars.ajax_url ,
				data: {action:'emd_formb_submit_ajax_form',form_data:form_data,nonce:emd_form_vars.nonce},
