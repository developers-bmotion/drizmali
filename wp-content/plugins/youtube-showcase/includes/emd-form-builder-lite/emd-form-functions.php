...
if (!function_exists('emd_form_builder_submit_form')) {
	function emd_form_builder_submit_form($app, $fcontent) {
		...
		$ret_attrs = emd_form_builder_check_attr($fcontent['layout'],$fcontent['entity'],$ent_list,$attr_list,$txn_list,$rel_list);
		...
		if (!empty($id)) {
			...
			if (!empty($ret_attrs['file_attrs'])) {
				emd_form_builder_unset_file_upload($session_class);
			}
			return $ret;
		}
		if (!empty($ret_attrs['file_attrs'])) {
...
function emd_form_builder_check_attr($layout,$fentity,$ent_list,$attr_list,$txn_list=Array(),$rel_list=Array(),$glob_list=Array()){
	$ret = Array();
	if(!empty($layout)){
		foreach($layout as $kpage => $mypage){
			foreach($mypage['rows'] as $myrow){
...
add_action('wp_ajax_nopriv_emd_form_builder_pagenum', 'emd_form_builder_pagenum');
...
add_filter('kses_allowed_protocols', 'emd_form_allow_data_protocol_urls', 10);
function emd_form_allow_data_protocol_urls($allowed_protocols) {
